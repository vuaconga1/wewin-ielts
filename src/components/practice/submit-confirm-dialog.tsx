  ...
  return (
    ...
    >
      <div className="w-full max-w-md border border-zinc-500 bg-[#f0f1f3] shadow-xl">
        <div className="border-b border-zinc-400 bg-[#2c3645] px-4 py-2.5">
          <h2
            id="cdi-submit-title"
...
export function TestEndedOverlay({ visible }: { visible: boolean }) {
  const { t } = useTranslations("practice");
  if (!visible) return null;
  return (
    <div
      className="fixed inset-0 z-[60] flex flex-col items-center justify-center gap-3 bg-[#1e2430] px-6 text-center"
      role="status"
      aria-live="polite"
    >
      ...
      <p className="max-w-sm text-sm text-zinc-300">
        {t("testEndedHint", "Your answers have been submitted. Loading results…")}
      </p>
      <div
        className="mt-2 h-1 w-40 overflow-hidden rounded-full bg-zinc-700"
        aria-hidden
      >
